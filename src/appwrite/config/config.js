...
export class Service {
    ...
    async updatePost(slug, { title, content, featuredImage, status }) {
        try {
            return await this.databases.updateDocument(
                conf.appwriteDatabaseId,
                conf.appwriteCollectionId,
                slug,//we are using slug as document id
                {
                    title,
                    content,
                    featuredImage,
    ...
    async deletePost(slug) {
        try {
            ...
            return true;//return true if the post is deleted
        } catch (error) {
    ...
    getFilePreview(fileId) {//this function will return the file preview url
        //we are not using async await here because we are not making any api call
        try {
